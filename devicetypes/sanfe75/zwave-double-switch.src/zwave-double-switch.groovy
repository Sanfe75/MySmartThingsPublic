 ...
 */
metadata {
	definition (name: "Z-Wave Double Switch", namespace: "Sanfe75", author: "Simone", ocfDeviceType: "oic.d.switch") {
		capability "Actuator"
	...
	preferences {
        ...
        input "secondarySwitch", "capability.switch", multiple: true, required: false, title: "Select"
    }
...
def parse(String description) {
	...
	if (result?.name == 'hail' && hubFirmwareLessThan("000.011.00602")) {
        ...
        if (secondarySwitch) {
            if (result?.value == "on") {
                log.debug "result?.value= ${result?.value}"
                secondarySwitch.on()
